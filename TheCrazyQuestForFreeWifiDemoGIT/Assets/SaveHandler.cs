...
public class SaveHandler : MonoBehaviour
{
    ...
    public void Save(string datastr)
    {
        ...
        {
            File.WriteAllText(Application.persistentDataPath + "/save1.txt", datastr);
        }
        else if (datastr.Substring(0, 1) == "2")
        {
            File.WriteAllText(Application.persistentDataPath + "/save2.txt", datastr);
        }
        else if (datastr.Substring(0, 1) == "3")
        {
            File.WriteAllText(Application.persistentDataPath + "/save3.txt", datastr);
        }
        else
        {
            File.WriteAllText(Application.persistentDataPath + "/savefail.txt", "failed to save, heres the data: " + datastr);
        }
    ...
    public void Load1()
    {
        string savedata = File.ReadAllText(Application.persistentDataPath + "/save1.txt");
        filenum = "1";
    ...
    public void Load2()
    {
        string savedata = File.ReadAllText(Application.persistentDataPath + "/save2.txt");
        filenum = "2";
        PlayerPrefs.SetString("file", filenum);
        plrlvl = savedata.Substring(2, 1);
        world = savedata.Substring(4, 2);
    ...
    public void Load3()
    {
        string savedata = File.ReadAllText(Application.persistentDataPath + "/save3.txt");
        filenum = "3";
        PlayerPrefs.SetString("file", filenum);
        plrlvl = savedata.Substring(2, 1);
        world = savedata.Substring(4, 2);
